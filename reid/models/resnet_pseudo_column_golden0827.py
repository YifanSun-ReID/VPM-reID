...
#                yy[i] = F.adaptive_avg_pool2d(yy[i],(ratio[i],1))
                xx[i] = F.avg_pool2d(xx[i],(stride[i],8))
#                xx[i] = F.adaptive_avg_pool2d(xx[i],(3,1))     # 1X2048 X H X W
                if xx[i].size(2)<6:
#                    yy[i] = torch.cat((yy[i],torch.cat(batch_pad_y*(6-ratio[i]),2)),2)
                    xx[i] = torch.cat((xx[i],torch.cat(batch_pad_x*(6-xx[i].size(2)),2)),2)
