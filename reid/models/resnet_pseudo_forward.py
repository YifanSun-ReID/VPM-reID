...
import torchvision
import torch
__all__ = ['resnet50_pseudo_forward']
import numpy as np
...
#            y = y-center.expand_as(y)
            ...
            y = F.avg_pool2d(y,(4,8))            


            local_score = self.local_mask(y).squeeze(3)
            local_prob = self.local_mask(z)
            local_prob = F.softmax(local_prob,1)
            max_prob, indice = F.max_pool2d(local_prob, (24,3), return_indices=True)
            indice = indice.squeeze(3).squeeze(2)
#            local_score = local_score.squeeze(3)
            tmp = local_score
            patch = F.avg_pool2d(x,kernel_size=(3,6),stride=(1,1),padding=(1,0))
            patch = patch.resize(patch.size(0),patch.size(1),24*3)
            x = F.avg_pool2d(x,(4,8))

            indx_sample = np.zeros([local_score.size(0),6])
            for i in range(0,local_score.size(0)):
                indx_sample[i] = i
#            indx_sample = indx_sample.reshape(local_score.size(0)*6)
            indx_sample = indx_sample.astype('int')
            spand_indice = indice.resize(indice.size(0)*6)
#            indx_part = spand_indice.cpu().numpy()
            indx_part = indice.cpu().numpy()
            feature = patch[indx_sample, :, indx_part]
#            feature = feature.resize(local_score.size(0),6*2048)
...
#==================================================================================#
            ...
            out0 = feature/feature.norm(2,1).unsqueeze(1).expand_as(feature)
            x = feature.unsqueeze(3) 
            x = self.drop(x)
            x = self.local_conv(x)
...
def resnet50_pseudo_forward(**kwargs):
