...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
                 num_features=0, norm=False, dropout=0, num_classes=0, FCN=False, radius=-1.0, thresh=0.5):
...
#===================================================================#
            ...
            self.radius = torch.autograd.Variable(torch.FloatTensor([-4.1870]).cuda())
            self.radius = torch.autograd.Variable(torch.FloatTensor([-5.0469]).cuda())   #  step1/160 epochs
#            self.radius = torch.autograd.Variable(torch.FloatTensor([-1.8585]).cuda())   #  step2/190 epochs
#            self.radius = torch.autograd.Variable(torch.FloatTensor([-4.9095]).cuda())   #  step1/160 epochs hardest
#            self.radius = torch.autograd.Variable(torch.FloatTensor([-4.9732]).cuda())   #  GSContrastive
#            self.radius = torch.autograd.Variable(torch.FloatTensor([-2.0578]).cuda())   #  GSContrastive_Plus_SMLoss
#            self.radius = torch.autograd.Variable(torch.FloatTensor([-1.9942]).cuda())   #  step2/180 epochs hardest
#            self.radius = torch.autograd.Variable(torch.FloatTensor([-1.2828]).cuda())   #  step2/220 epochs
  
            self.radius = torch.autograd.Variable(torch.FloatTensor([-4.5663]).cuda())   #  DUKE 120 epochs ag0312
            self.radius = torch.autograd.Variable(torch.FloatTensor([-1]).cuda())
            self.radius = torch.autograd.Variable(torch.FloatTensor([-7.321]).cuda())
            self.radius = torch.autograd.Variable(torch.FloatTensor([-16.0]).cuda())
            ...
            self.thresh.requires_grad = True
            out_planes = self.base.fc.in_features
...
#        return out3, x, out4, out3
#           representation, category, tripletloss, SMLoss
        return out2, x, out2, out2, self.radius 
#        return out3, x, out3, out3, self.radius 
