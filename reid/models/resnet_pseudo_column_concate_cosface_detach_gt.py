...
class ResNet(nn.Module):
    ...
    def forward(self, inputs, part_labels=None):
        ...
        if part_labels is None:
        	part_visibility = torch.ones(x.size(0), self.num_parts).cuda()
        else:
        	part_visibility = (part_labels[:, 0:-1]!=751).float().detach() # b*6
        # if part_labels is None:
        ...
        if self.FCN:
            ...
            part_features = f[0:-1]
            part_features = torch.cat([i.unsqueeze(1) for i in part_features], dim=1)
            part_features = part_visibility.unsqueeze(2)*part_features
            part_features = part_features.view(part_features.size(0), -1)
            c = []
            c.append(self.part_classifier(part_features))
...
def resnet50_pseudo_column_concate_cosface_detach_gt(**kwargs):
