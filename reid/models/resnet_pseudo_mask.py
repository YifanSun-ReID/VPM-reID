...
#====================================================================================
            ...
            pw = local_mask.sum(3).sum(2).mean(0)
            num_parts = min(pw.gt(25).sum().item()+1,6)
            print(num_parts)
            ff = F.adaptive_avg_pool2d(x,(num_parts,1))
...
#            f5 = f5.unsqueeze(2).unsqueeze(3) 
            ...
            feat = torch.cat((f0,f1,f2,f3,f4),2)
            if ff.size(2)<4:
                ff = torch.cat([ff,torch.zeros(64,2048,4,1).cuda()],2)   # in case distractors only get 1 or 0 part
            feat = ff[:,:,0:5,:]
...
#            c7 = self.instance5(x[7].contiguous().view(x[7].size()[0],-1))
            return out0, (c0, c1, c2, c3, c4, c5), (ps0, ps1, ps2, ps3, ps4, ps5)#, pool5#, orig_weight, pool5#, (x0, x1, x2, x3, x4, x5) #, glob#, c6, c7
...
def resnet50_pseudo_mask(**kwargs):
