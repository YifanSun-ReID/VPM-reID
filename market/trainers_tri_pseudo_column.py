...
from .loss import OIMLoss, TripletLoss, PartialTripletLoss
...
class BaseTrainer(object):
    def __init__(self, model, criterion, X, Y, SMLoss_mode=0, Triplet_margin=1.0):
...
#            sdegrees.update(sdegree.data[0],targets.size(0))
            ...
            optimizer.zero_grad()
            weight_ploss = min((epoch/10)/10., 1.0)
            #ploss = ploss*weight_ploss
            total_loss = loss + [gloss, ploss, Tloss]
