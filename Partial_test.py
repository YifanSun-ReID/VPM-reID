...
def  main(args):
    ...
    if args.height is None or args.width is None:
        args.height, args.width = (144, 56) if args.arch == 'inception' else \
                                  (256, 128)
    dataset, num_classes, query_loaders, gallery_loader = \
        get_data(args.dataset, args.data_dir, args.height, args.width, args.ratios, args.batch_size, args.workers)
    ...
    for i, ratio in enumerate(args.ratios):
        print("Test ratio {}:".format(ratio))
        with torch.no_grad():
            evaluator.evaluate(query_loaders[i], gallery_loader, dataset.query, dataset.gallery)
    return	
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ratios', type=float, nargs='+', default=[0.5, 0.6, 0.7, 0.8, 0.9, 1.0])
